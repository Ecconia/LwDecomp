...
namespace LwDecomp
{
	internal class Program
	{
		static void Main(string[] args)
		{
			...
			DecompDirectory(
				Path.Combine(decompRootFolder, "LWServer"), 
				Path.Combine(lwInstallPath, serverSubpath), 
				files => files.Where(f => f.EndsWith(".dll") && !skipPrefixes.Any(keyword => f.StartsWith(keyword)))
			);
			
			Console.WriteLine("\nDecompiling client...");
			DecompDirectory(
				Path.Combine(decompRootFolder, "LWClient"), 
				Path.Combine(lwInstallPath, clientSubpath), 
				files => files.Where(f => f.EndsWith(".dll") && !skipPrefixes.Any(keyword => f.StartsWith(keyword)))
			);
